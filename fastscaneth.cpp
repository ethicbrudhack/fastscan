// ==========================
// FASTSCAN v2 – TYLKO ETH!
// Z 24-BITOWYM INDEKSEM (128 MB) - SZYBKI!
// ==========================
...
// TYLKO ETH HASH!
// ============================================================
inline void get_eth_hash(const FastPubCtx& pc, unsigned char eth_hash[20]) {
    unsigned char pub[65]; 
    size_t pub_len = 65;
    secp256k1_ec_pubkey_serialize(pc.ctx, pub, &pub_len, &pc.pub, 
                                  SECP256K1_EC_UNCOMPRESSED);
    
    unsigned char keccak_tmp[32];
    keccak_256_fixed(pub + 1, 64, keccak_tmp);
    memcpy(eth_hash, keccak_tmp + 12, 20);
}

// ===============================
// BASE58 (Tylko dla ETH - nie potrzebne, ale zostawiam)
// ===============================
...
std::string addr_eth(const unsigned char eth_hash[20]) {
    std::string result = "0x";
    for (int i = 0; i < 20; i++) {
        char hex[3];
        sprintf(hex, "%02x", eth_hash[i]);
        result += hex;
    }
    return result;
}
...
// ===============================
// SKANOWANIE - TYLKO ETH!
// ===============================
inline void scan_key_fast(FastPubCtx& fctx, const unsigned char priv[32], const PrefixIndex24& idx) {
    // ==========================================
    // TYLKO ETH HASH
    // ==========================================
    unsigned char eth_h[20];
    get_eth_hash(fctx, eth_h);
    
    // ==========================================
    // SPRAWDŹ TYLKO ETH
    // ==========================================
    if (idx.contains(eth_h)) {
        std::string addr = addr_eth(eth_h);
        save_found(addr, priv, "ETH");
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        std::cout << "Użycie: ./scan_eth adresy.bin start_bit end_bit [--resume]\n";
        return 1;
    }
